...
pipeline {
    ...
    stages {
        ...
        stage('Checkout Cloud-Native SG Git Repository') {
            steps {
                git branch: 'master',
                        url: '${GERRIT_CENTRAL}/OSS/com.ericsson.oss.containerisation/eric-enmsg-amos-odp'
            }
        }
...
//                dir('amos') {
//                    git branch: 'master',
//                            url: '${GERRIT_CENTRAL}/OSS/com.ericsson.oss.servicegroupcontainers/AMOS'
//                }
//            }
//        }
        stage('Helm Dep Up ') {
        ...
        stage('Merge values files') {
            steps{
                 script {
                     appconfig_values = sh (script: "ls ${WORKSPACE}/chart/eric-enmsg-amos-odp/appconfig/ | grep values.yaml", returnStatus: true)
                     ...
                     sh '''
                         if git status | grep 'values.yaml' > /dev/null; then
                            git add chart/eric-enmsg-amos-odp/values.yaml
